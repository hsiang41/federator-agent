...
func getTimeFromTo(starttime *timestamp.Timestamp, durationDays int) (int64, int64) {
	...
	du, _ := time.ParseDuration(fmt.Sprintf("%dd", durationDays))
	sTm = sTm.Add(du)
...
func (i inputLib) Gather() error {
	...
	for i := 0; i < 2; i++ {
		...
		logger.Info(fmt.Sprintf("Succeed to write recommendation fill_dys: %d, start time: %d, granularity: %d", fill_days, st, granularity))
	}
	return nil
}
...
func (i inputLib) LoadConfig(config string, scope *logUtil.Scope) error {
	...
	if err != nil {
		...
		if transmitterConfBin, err := json.MarshalIndent(fedConf, "", "  "); err == nil {
	...
	gFedermeter = NewCostAnalyst(&fedConf, scope)
	logger = scope
	return nil
}
...
func NewCostAnalyst(conf *CostAnalysisConf, logger *logUtil.Scope) *CostAnalyst {
	dp := DP.NewDataHubClient()
	dp.Scope = logger
	dp.DataHub.DataHub = conf.DataHub
	fed := FedAPI.NewFedermeter(conf.Fedemeter.Url, conf.Fedemeter.Username, conf.Fedemeter.Password, logger)
	return &CostAnalyst{Conf: conf, Scope: logger, DPClient: dp, FedApi: fed}
}
